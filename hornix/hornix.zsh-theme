...
% $(prompt_char)%{$reset_color%} '
	...
	PROMPT='$fg_bold[yellow][%D{%H:%M:%S %d/%m/%y}|$fg_bold[cyan]${OPS}] $(user) %{$fg_bold[blue]%}%(!.%1~.%~) $(git_prompt_info)
% $(prompt_char)%{$reset_color%} '
elif [[ ${OPS} == "FreeBSD" ]] || [[ ${OPS} == "Scientific Linux" ]] || [[ ${OPS} == "Ubuntu" ]] ; then
	PROMPT='$fg_bold[yellow][%D{%H:%M:%S %d/%m/%y}|$fg_bold[red]${OPS}] $(user) %{$fg_bold[blue]%}%(!.%1~.%~) $(git_prompt_info)
% $(prompt_char)%{$reset_color%} '
elif [[ ${OPS} == "Arch Linux" ]] || [[ ${OPS} == "Fedora" ]] || [[ ${OPS} == "Mageia" ]]; then
	PROMPT='$fg_bold[yellow][%D{%H:%M:%S %d/%m/%y}|$fg_bold[blue]${OPS}] $(user) %{$fg_bold[blue]%}%(!.%1~.%~) $(git_prompt_info)
% $(prompt_char)%{$reset_color%} '
elif [[ ${OPS} == "Gentoo Linux" ]]; then
	PROMPT='$fg_bold[yellow][%D{%H:%M:%S %d/%m/%y}|$fg_bold[purple]${OPS}] $(user) %{$fg_bold[blue]%}%(!.%1~.%~) $(git_prompt_info)
% $(prompt_char)%{$reset_color%} '
elif [[ ${OPS} == "Void" ]]; then
	PROMPT='$fg_bold[yellow][%D{%H:%M:%S %d/%m/%y}|$fg_bold[white]${OPS}] $(user) %{$fg_bold[blue]%}%(!.%1~.%~) $(git_prompt_info)
% $(prompt_char)%{$reset_color%} '
elif [[ $(uname) == "Linux" ]]; then
	PROMPT='$fg_bold[yellow][%D{%H:%M:%S %d/%m/%y}|$fg_bold[yellow]${OPS}] $(user) %{$fg_bold[blue]%}%(!.%1~.%~) $(git_prompt_info)
% $(prompt_char)%{$reset_color%} '
fi
